      ...
      objetoSelecionado = obj_Circulo;
        ...
        protected override void OnKeyDown(OpenTK.Input.KeyboardKeyEventArgs e)
        {
            if (e.Key == Key.H){
                ...
                Exit();
            }else if (e.Key == Key.X){//mudamos de E para X
                Console.WriteLine("--- Objetos / Pontos: ");
                for (var i = 0; i < objetosLista.Count; i++)
                {
                    Console.WriteLine(objetosLista[i]);
                }
            }else if (e.Key == Key.Z){//mudamos de O para Z, devido conflito com zoomOut
                bBoxDesenhar = !bBoxDesenhar;
            }else if (e.Key == Key.V){
                mouseMoverPto = !mouseMoverPto;   //TODO: falta atualizar a BBox do objeto
            }else if (e.Key == Key.I){
              //verifica para nao ocorrer inversao
                if(((camera.xmax - 2) > 0) && ((camera.xmin+2) < 0) && ((camera.ymin +2) < 0) && ((camera.ymax -2 ) >0))
                  camera.ZoomIn();
            }else if (e.Key == Key.O)
            {
                camera.ZoomOut();
